		...
		function addChat(chatname,userchat,chattime,chatID){
			if(chatname=="<%=ownerId%>owner"){
				$('#chatList').append('<p><ul><li class="rights" id="'+
					...
					'</span>'+
					'<font size=1><br>'+
					'<input style="WIDTH:4%;Height:4%; float:center" type="submit" value="x" onclick="deletechat('+
					chatID+
					')">'+
					chattime+
					'</font><br></li></ul></p>'
					);
			}else if(chatname=="<%=ownerId%>finder")
				{
			...
			else if(chatname=="<%=ownerId%>pwner"){
			...
			else if(chatname=="<%=ownerId%>pfinder"){
...
<body>
    <div class="chat_wrap">
        ...
        <div class="input-div">
			<textarea cols="23" rows="3" wrap="hard" placeholder="메시지를 입력하세요." name="message" id="message"></textarea>
			<form target="iframe1" action="fileOK.jsp" style="display:none" method="post" enctype="multipart/form-data">
   ...
   <Script>function ifun(msg){<!--fileOK.jsp에서 날라온 메세지 띄우기-->
        ...
        <input class="a" type="submit" value="사진 전송" onclick="$('#filebutton').trigger('click');">
        <input class="b" type="submit" value="  전송      " onclick="submitFunction()">
    </div>
